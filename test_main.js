...
function renderCollectionList(container, collections) {
    ...
    // Determine database type from global config or current config
    let dbType = window._dbType;
    if (!dbType && currentDbConfig) {
        dbType = currentDbConfig.pinecone && currentDbConfig.pinecone.api_key ? 'pinecone' : 'milvus';
    }
    if (!dbType) dbType = 'milvus'; // Default fallback
...
window.openDbConfigModal = async function () {
    ...
    if (dbTypeSelect) {
        ...
        // Add event listener for type switching
        dbTypeSelect.onchange = () => toggleConfigSections(dbTypeSelect.value);
    }

    // Support both Pinecone and Milvus configurations
    if (isPinecone) {
        // Pinecone configuration
        const cfgUriEl = document.getElementById('cfg-uri');
        const cfgTokenEl = document.getElementById('cfg-token');
        
        if (cfgUriEl) cfgUriEl.value = `Pinecone://${cfg.pinecone.index_name || 'default'}`;
        if (cfgTokenEl) cfgTokenEl.value = cfg.pinecone.api_key;
        
        // Fill Pinecone specific fields
        const pineconeIndexInput = document.getElementById('cfg-pinecone-index');
        if (pineconeIndexInput) {
            pineconeIndexInput.value = cfg.pinecone.index_name || '';
        }
        const pineconeDimensionInput = document.getElementById('cfg-pinecone-dimension');
        if (pineconeDimensionInput) {
            pineconeDimensionInput.value = cfg.pinecone.dimension || 384;
        }
        const pineconeMetricSelect = document.getElementById('cfg-pinecone-metric');
        if (pineconeMetricSelect) {
            pineconeMetricSelect.value = cfg.pinecone.metric || 'cosine';
        }
        
        // Mark as Pinecone mode
        ...
        if (cfgUriEl) cfgUriEl.value = milvus.uri || '';
        if (cfgTokenEl) cfgTokenEl.value = milvus.token || '';
        window._dbType = 'milvus';
    }

    // Toggle config sections based on type
    toggleConfigSections(dbType);
    ...
    if (dbConfigModal) dbConfigModal.showModal();
};

// Helper function to toggle between Pinecone and Milvus config sections
function toggleConfigSections(dbType) {
    const pineconeSection = document.getElementById('pinecone-config-section');
    const milvusSection = document.getElementById('milvus-config-section');
    
    if (!pineconeSection || !milvusSection) return;
    
    if (dbType === 'pinecone') {
        pineconeSection.style.display = 'block';
        milvusSection.style.display = 'none';
    } else {
        pineconeSection.style.display = 'none';
        milvusSection.style.display = 'block';
    }
}

window.saveDbConfig = async function () {
    const dbTypeSelect = document.getElementById('cfg-db-type');
    const dbType = dbTypeSelect ? dbTypeSelect.value : (window._dbType || 'milvus');
    ...
    if (dbType === 'pinecone') {
        ...
        const indexInput = document.getElementById('cfg-pinecone-index');
        ...
        const indexName = indexInput ? indexInput.value.trim() : 'default';
        const dimension = dimensionInput ? parseInt(dimensionInput.value) : 384;
        const metric = metricSelect ? metricSelect.value : 'cosine';
        
        if (!apiKey) { 
            showModal("Pinecone API Key 是必填项", { title: t("kb_validation_error"), type: "warning" }); 
            return; 
        }
        ...
        // Update URI display for Pinecone
        const cfgUriEl = document.getElementById('cfg-uri');
        const cfgTokenEl = document.getElementById('cfg-token');
        if (cfgUriEl) cfgUriEl.value = `Pinecone://${indexName}`;
        if (cfgTokenEl) cfgTokenEl.value = apiKey;
    ...
    // Update global DB type
    window._dbType = dbType;
